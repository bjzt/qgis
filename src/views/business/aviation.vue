    ...
    <el-form :inline="true">
      ...
      <el-form-item label="选择相机">
        <el-select v-model="camera" @change="cameraChange">
          ...
          <el-option v-for="item in cameraList" :label="item.name" :value="item.id" :key="item.id"></el-option>
        </el-select>
      </el-form-item>
    </el-form>
    ...
    oldItem: {
      ...
      handler(val) {
        ...
        if (val.dataType == 0) {
          ...
          if (
            val.course < standard.course ||
            val.sideDirection < standard.sideDirection
          ) {
            this.$message({
              type: "warning",
