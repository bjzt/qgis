    ...
    <el-row >
      ...
      <el-col :span="8" style="border: 1px solid rgba(0, 0, 0, .12)">
        ...
        <el-form size="mini">
          ...
          <el-form-item v-if="model == 0" style="margin:0" label="z偏移(m) = " :label-width="labelWdth">
            <el-input v-model.number="sevenPar.dZ"></el-input>
          </el-form-item>

          <el-form-item v-if="model == 1" style="margin:0" label="旋转量 dms = " :label-width="labelWdth">
      ...
      <el-form size="mini">
        <el-row>
          ...
          <el-col>
            <table>
              <tr>
                <td style="border: 2px solid black;padding:5px" v-for="(item,index) of lieList" :key="index" v-text="item.label"></td>
              </tr>
            </table>
          </el-col>
        </el-row>
      ...
      <div style="text-align: center;height: 300px">
        <el-scrollbar class="scrollbar" style="height: 100%">
          ...
          <table v-if="header0" class="table">
            ...
            <tr>
              <td>原始坐标</td><td>{{item1.zb}}</td><td>目标坐标</td><td>{{item2.zb}}</td>
            </tr>
            <tr>
              <td>坐标格式</td>
            ...
            <tr>
              <td>投影方式</td><td>{{item1.zyzw}}</td><td>投影方式</td><td>{{item2.zyzw}}</td>
            </tr>
            <tr>
              <td>中央经线</td>
...
export default {
  data() {
    ...
    return {
      ...
      oldData: [],//转换前数据
      newData: [],//转换后数据
      exportFormatList: [{value:"txt"},{value:"dat"},{value: "excel"}],//导出的文件格式列表
      exportFormat: "",
      exportVisible: false,//导出的开关
      header: null,//表头项
      headerOptions: [{label: "参数设定", value: 0},{label: "参数说明",value: 1}],//表头选项
      header0: false,//参数设定
      header1: false,//参数说明
      gg: false,//杆高
  ...
  methods: {
    ...
    selectModel(value){
      this.paramCE()
      switch(value){
        case 0: this.modelName = "布尔莎七参数";break
        case 1: this.modelName = "四参数";break
    ...
    printJson(dto){
      ...
      for(let lie of this.lieList){
        switch(lie.value){
          case "0": str += dto.name;break//点名
          case "1": str += dto.x;break//源x
          case "2": str += dto.y;break//源y
          case "3": str += dto.h;break//源H
          // case "4": str += dto.newX;break
          // case "5": str += dto.newY;break
          // case "6": str += dto.newH;break
          case "7": str += dto.code;break//编码
          case "8": str += dto.node;break//备注
          case "9": str += dto.other;break//其它
          case "10": str += dto.yaw;break
          case "11": str += dto.pitch;break
          case "12": str += dto.roll;break
          case "13": str += dto.salt;break
          case "14": str += dto.gh;break
        }
        str +=this.symbol
      }
    ...
    addDataToTable1(){
      ...
      for (let j=start;j< this.showData.length;j++){
        ...
        for (let i=0;i< this.lieList.length;i++) {
          switch(this.lieList[i].value){
            case "0": dto.name = data[i];break
            case "1": dto.x = data[i];break
            case "2": dto.y = data[i];break
            case "3": dto.h = data[i];break
            case "4": dto.newX = data[i];break
            case "5": dto.newY = data[i];break
            case "6": dto.newH = data[i];break
            case "7": dto.code = data[i];break
            case "8": dto.node = data[i];break
            case "9": dto.other = data[i];break
            case "10": dto.yaw = data[i];break
            case "11": dto.pitch = data[i];break
            case "12": dto.roll = data[i];break
            case "13": dto.salt = data[i];break
            case "14": dto.gh = data[i];break
          }
        }
    ...
    computer(){
      ...
      if (sevenPar.dX == null || sevenPar.dY == null || sevenPar.dZ == null || 
        sevenPar.Cita_X == null || sevenPar.Cita_Y == null || sevenPar.Cita_Z == null || sevenPar.k == null) {
        this.$message({
          type: "error",
          message: "7参数不完整"
        })
