<template>
  <div style="margin: 20px">
    <el-row>
      ...
      <el-col :span="20">
        ...
        >
          ...
          <el-table-column prop="factorType" label="要素类型" width="150">
        ...
        <el-table
          ...
          style="width: 100%"
        >
          <el-table-column prop="count" label="主要地物总数" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.count}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="joinCount" label="参与中误差统计点数" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.joinCount}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="pointError" label="地物点位中误差" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.pointError}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="errorDiffe" label="中误差限差" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.errorDiffe}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="notUseNumber" label="粗差个数" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.notUseNumber}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="notUseScale" label="粗差率" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.notUseScale}}</span>
            </template>
          </el-table-column>
          <el-table-column prop="diffe" label="限差" width="150">
            <template slot-scope="scope">
        ...
        >
          ...
          <el-table-column prop="oldLength" label="原测相对位置(图上距离)" width="150">
            <template slot-scope="scope">
              <el-input size="mini" v-if="scope.row.status" v-model="scope.row.oldLength"></el-input>
              <span v-else v-text="scope.row.oldLength"></span>
            </template>
          </el-table-column>
          <el-table-column prop="newLength" label="检测相对位置(实测距离)" width="150">
            <template slot-scope="scope">
              <el-input size="mini" v-if="scope.row.status" v-model="scope.row.newLength"></el-input>
              <span v-else v-text="scope.row.newLength"></span>
            </template>
          </el-table-column>
          <el-table-column prop="length_C" label="较差(ds)" width="150">
            <template slot-scope="scope">
              <span v-text="scope.row.length_C"></span>
            </template>
          </el-table-column>
          <el-table-column prop="isNotUse" align="center" label="可用">
            <template slot-scope="scope">
              <span v-if="scope.row.isNotUse">粗差</span>
            </template>
          </el-table-column>
          <el-table-column prop="isUse" align="center" fixed="right" label="采用">
            <template slot-scope="scope">
              <div style="width:100%;height:23px" @click="scope.row.isUse = !scope.row.isUse">
                <span v-if="scope.row.isUse">√</span>
              </div>
          ...
          <el-table-column prop="status" align="center" width="150" fixed="right" label="操作">
            <template slot-scope="scope">
              <el-button-group>
                <el-button size="mini" @click="delItem(scope.$index)" type="danger">删除</el-button>
                <el-button
        ...
        <el-table
          ...
          size="small"
        >
          <el-table-column prop="count" label="相关边长总数" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.count}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="pointError" label="相关边长间距中误差" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.pointError}}</span>
            </template>
          </el-table-column>
        ...
        >
          ...
          <el-table-column prop="oldH" label="原高程h" width="150">
            <template slot-scope="scope">
        ...
        <el-table
          ...
          size="small"
        >
          <el-table-column prop="count" width="150" label="等高线内插高程注记总数">
            <template slot-scope="scope">
              <span>{{scope.row.count}}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="pointError" label="高程注记中误差" width="150">
            <template slot-scope="scope">
              <span>{{scope.row.pointError}}</span>
            </template>
          </el-table-column>
        ...
        >
          <el-table-column type="index" width="50"></el-table-column>
          <el-table-column prop="mapNumber" align="center" label="图幅号">
            <template slot-scope="scope">
              <span>{{scope.row.mapNumber}}</span>
            </template>
          </el-table-column>
          <el-table-column align="center" label="数学精度">
            <el-table-column prop="mathScore" align="center" label="数学基础评分"></el-table-column>
            <el-table-column align="center" label="平面精度Ms检">
              <el-table-column prop="absolute" align="center" label="绝对位置">
                <template slot-scope="scope">
                  <span>{{scope.row.absolute}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="absoluteScore" align="center" label="得分">
                <template slot-scope="scope">
                  <span>{{scope.row.absoluteScore}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="relative" align="center" label="相对位置">
                <template slot-scope="scope">
                  <span>{{scope.row.relative}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="relativeScore" align="center" label="得分">
                <template slot-scope="scope">
                  <span>{{scope.row.relativeScore}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="planeScore" align="center" label="平面精度评分">
                <template slot-scope="scope">
                  <span>{{scope.row.planeScore}}</span>
                </template>
              </el-table-column>
            </el-table-column>
            <el-table-column align="center" label="高程精度Mh检">
              <el-table-column prop="elevation" align="center" label="高程注记">
                <template slot-scope="scope">
                  <span>{{scope.row.elevation}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="elevationScore" align="center" label="得分">
                <template slot-scope="scope">
                  <span>{{scope.row.elevationScore}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="contour" align="center" label="等高线内插高程注记">
                <template slot-scope="scope">
                  <span>{{scope.row.contour}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="contourScore" align="center" label="得分">
                <template slot-scope="scope">
                  <span>{{scope.row.contourScore}}</span>
                </template>
              </el-table-column>
              <el-table-column prop="altitudeScore" align="center" label="高程精度评分">
                <template slot-scope="scope">
                  <span>{{scope.row.altitudeScore}}</span>
                </template>
              </el-table-column>
            </el-table-column>
            <el-table-column prop="math" align="center" label="数学精度评分">
              <template slot-scope="scope">
      ...
      <div v-else>
        <table style="text-align: center;border-collapse: collapse;">
          ...
          >
            ...
            <td
              style="border: 2px solid #cad9ea;color: #666;padding:5px"
              v-for="item in itemList.dataList"
              :key="item"
              v-text="item"
            ></td>
          </tr>
    ...
    <el-dialog title="随机的比例" :visible.sync="randomVisible" width="30%">
      <el-form size="mini">
  ...
  methods: {
    ...
    randomData() {
      ...
      for (let item of this.tableData) {
        ...
        if (item.newH != null) {
          item.newH = parseFloat(item.newH).toFixed(3);
        }
        temp.push(item);
      }
      this.tableData = temp;
        ...
        case 1: {
          ...
          if (this.computeData[0].notUseScale > 5) {
            ...
            if (error > 0.3) {
              absoluteScore =
                60 +
                (40 / (this.oldItem.standardValue * 0.7)) *
                  (this.oldItem.standardValue - error);
            } else {
              absoluteScore = 100;
            }
    ...
    menu72(flag) {
      ...
      if (
        this.oldItem.terrainType != null &&
        this.oldItem.scaleSurvey != null
      ) {
